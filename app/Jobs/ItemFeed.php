...
class ItemFeed implements ShouldQueue
{
    use Dispatchable, 
    InteractsWithQueue, Queueable, SerializesModels , CategoryHelper, ImageUploader, ImageHelper, RegionHelper;
    ...
    public function __construct($feed , $url)
    {
        //
        $this->url = $url ;
        $this->feed = $feed ;
...
//                                                 @$doc->loadHTML($link_page);
    ...
    if (isset($body_class) && !empty($body_class)) {
        ...
        if (isset($bodys) && !empty($bodys) && count($bodys) > 0) {
            ...
            /* Body Here */
       $body = strip_tags($body , '<p><h2><h3><h4><h5><h6><table><tr><th><td><span><ul><li>');
...
if(isset($body) && !empty($body)){
    ...
    if (isset($title) && !empty($body) && !empty($title)) {
        ...
        $this->set_new($feed, $title, $body, $images);
    }
