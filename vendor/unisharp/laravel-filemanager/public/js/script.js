    ...
    .done(function (data) {
      ...
      if (hasItems) {
        ...
        items.forEach(function (item, index) {
          ...
          if (item.thumb_url) {
            var image = $('<div>').css('background-image', 'url("storage/' + item.thumb_url + '?timestamp=' + item.time + '")');
          } else {
...
function preview(items) {
  ...
  items.forEach(function (item, index) {
    ...
    if (item.thumb_url) {
      carouselItem.find('.carousel-image').css('background-image', 'url(\'storage/' + item.url + '?timestamp=' + item.time + '\')');
    } else {
